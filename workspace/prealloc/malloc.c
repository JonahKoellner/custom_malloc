...
//	newZone = mmap(NULL, sizeof(t_storage), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
//	if (newZone == MAP_FAILED) {
//		return (NULL);
//	}
//	if ((newZone->ptr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)) == MAP_FAILED) {
//		munmap(newZone, sizeof(t_storage)); // free created zone if there was a error
//		return (NULL);
//	}
//	newZone->size = size;
//	newZone->next = NULL;
...
//	newBlock = mmap(NULL, sizeof(t_storage), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
//void _freeZone(t_storage *zone) {
//	munmap(zone->ptr, zone->size); // free created zone if there was a error
//	munmap(zone, sizeof(t_storage)); // free created zone if there was a error
//}

//// Special free function only for freeing Blocks
//void _freeBlock(t_storage *block) {
//	munmap(block, sizeof(t_storage)); // free created zone if there was a error
//}
...
//	t_storage *newZone = _createZone(zone_size);
